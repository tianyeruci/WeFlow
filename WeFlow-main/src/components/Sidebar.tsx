...
import { Home, MessageSquare, BarChart3, FileText, Settings, Download, Aperture, UserCircle, Lock, LockOpen, ChevronUp, FolderClosed, Footprints, Users, ArchiveRestore, Sparkles, Network } from 'lucide-react'
    ...
    <>
      <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
        <nav className="nav-menu">
          ...
          <NavLink
            to="/invite-stats"
            className={`nav-item ${isActive('/invite-stats') ? 'active' : ''}`}
            title={collapsed ? '邀请统计' : undefined}
          >
            <span className="nav-icon"><Network size={20} /></span>
            <span className="nav-label">邀请统计</span>
          </NavLink>
