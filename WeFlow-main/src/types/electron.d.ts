...
export interface InviteStatsGroupRow {
  group_id: string
  group_name: string
  avatar_url?: string
  member_count: number
...
export interface InviteMemberTraceRow {
  ...
  quit_type?: 'self_quit' | 'removed' | 'unknown'
  delete_flag: number
  valid_flag: number
  raw_content: string
...
export interface InviteStatsResult<T = unknown> {
  success: boolean
  data?: T
  error?: string
}

export interface InviteMemberTraceFilters {
  tagId?: string
  groupId?: string
  keyword?: string
  wxId?: string
...
export interface ElectronAPI {
  ...
  inviteStats: {
    listActivityTags: () => Promise<InviteStatsResult<InviteActivityTag[]>>
    saveActivityTag: (payload: { tagId?: string; tagName: string; enabled?: boolean }) => Promise<InviteStatsResult<InviteActivityTag>>
    setActivityTagEnabled: (tagId: string, enabled: boolean) => Promise<InviteStatsResult>
    deleteActivityTag: (tagId: string) => Promise<InviteStatsResult>
