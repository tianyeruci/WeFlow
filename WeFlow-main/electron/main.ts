...
function registerIpcHandlers() {
  ...
  // 邀请统计
  ipcMain.handle('inviteStats:listActivityTags', async () => {
    return inviteStatsService.listActivityTags()
  })

  ipcMain.handle('inviteStats:saveActivityTag', async (_, payload: { tagId?: string; tagName: string; enabled?: boolean }) => {
    return inviteStatsService.saveActivityTag(payload)
  })

  ipcMain.handle('inviteStats:setActivityTagEnabled', async (_, tagId: string, enabled: boolean) => {
    return inviteStatsService.setActivityTagEnabled(tagId, enabled)
  })

  ipcMain.handle('inviteStats:deleteActivityTag', async (_, tagId: string) => {
    return inviteStatsService.deleteActivityTag(tagId)
  })

  ipcMain.handle('inviteStats:listGroups', async () => {
    return inviteStatsService.listGroups()
  })

  ipcMain.handle('inviteStats:setGroupTag', async (_, groupId: string, tagId: string) => {
    return inviteStatsService.setGroupTag(groupId, tagId)
  })

  ipcMain.handle('inviteStats:clearGroupTag', async (_, groupId: string) => {
  ...
  ipcMain.handle('inviteStats:getDashboard', async (_, input: {
    tagId: string
    startTime?: number
    endTime?: number
  ...
  ipcMain.handle('inviteStats:getMemberTrace', async (_, filters: any) => {
    return inviteStatsService.getMemberTrace(filters)
  })

  ipcMain.handle('inviteStats:listPending', async (_, filters?: { tagId?: string }) => {
    return inviteStatsService.listPending(filters)
  })
  ...
  ipcMain.handle('inviteStats:ignorePending', async (_, payload: { eventType: 'invite' | 'quit'; eventId: string }) => {
    return inviteStatsService.ignorePending(payload)
  })

  ipcMain.handle('inviteStats:exportInviteRanking', async (_, payload: any) => {
    return inviteStatsService.exportInviteRanking(payload)
  })

  ipcMain.handle('inviteStats:exportMemberTrace', async (_, payload: any) => {
    return inviteStatsService.exportMemberTrace(payload)
  })
...
const shutdownAppServices = async (): Promise<void> => {
  ...
  shutdownPromise = (async () => {
    ...
    insightService.stop()
    inviteStatsService.stopAutoScanScheduler()
