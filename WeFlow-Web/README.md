...
- 邀请排行榜默认按邀请溯源记录计数：按溯源表展示的邀请者名称聚合，只要被邀请人记录存在于邀请溯源中就计 `1` 条，不因状态、归因有效性或是否退群而排除。
- 远程只读页展示默认不去重口径；桌面端开启“去重”时才按成员身份去重并沿用有效邀请口径。
- 群成员溯源的成员昵称搜索只匹配表格展示的成员昵称，不扩展到 wxid、邀请人或群名。
